Číselníky
=========

Číselník typu instituce
-----------------------

.. _api_jip:ciselnik:instituce:

.. list-table:: Ćíselník typu instituce
   :header-rows: 1
   
   * - Kód
     - Typ instituce
   * - 1
     - Česká pošta
   * - 2
     - Hospodářská komora
   * - 3
     - Krajský úřad
   * - 4
     - Ministerstvo
   * - 5
     - Obec I. Typu
   * - 6
     - Obec II. Typu
   * - 7
     - Obec III. Typu
   * - 8
     - Statutární města (Magistráty) a jejich obvody
   * - 9
     - Testovací
   * - 10
     - Notáři
   * - 11
     - Orgán státní správy
   * - 12
     - Školení
   * - 13
     - Exekutoři
   * - 15
     - Advokáti
   * - 16
     - Zřizované organizace
   * - 17
     - PO zřízená ze zákona
   * - 18
     - PO
   * - 19
     - PFO
   * - 20
     - FO
   * - 21
     - Stavební úřad
   * - 22
     - Orgán veřejné moci
   * - 23
     - Organizační složka státu
   * - 24
     - Daňoví poradci
   * - 25
     - Insolvenční správci


Číselník typu přihlášení
------------------------
     
.. _api_jip:ciselnik:prihlaseni:

.. list-table:: Ćíselník typu přihlášení
   :header-rows: 1

   * - Kód
     - Popis
     - Hodnota z datového modelu
   * - p-pwd
     - Přihlášení jménem a heslem
     - Při přihlášení s LoA nízká.
   * - p-cert
     - Přihlášení jménem, heslem a certifikátem
     - Při přihlášení s LoA značná.
   * - p-hotp
     - Přihlášení jménem, heslem a OTP kódem.
     - *Nevrací se.* OTP není v CAAIS implementováno.
   * - p-nia
     - Přihlášení prostřednictvím NIA.
     - Při přihlášení prostřednictvím NIA.

